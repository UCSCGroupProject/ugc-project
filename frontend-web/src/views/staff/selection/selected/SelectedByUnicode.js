    ...
    selectionService.getSelectedStudents().then(
      (res) => {
        if (res.type === 'OK') {
          ...
          const uniqueCourses = res.payload.filter(course => course.value1 == courseID).map(function (val) {
      ...
      <CRow>
        <CCol xs>
          <CCard className="mb-4">
            <CCardHeader>Students selected for {courseID}</CCardHeader>
            <CCardBody>
              <CRow>
                <div>
                  <CRow>
                                  ...
                                  <CTableRow key={item.id}>
                                    ...
                                    <CTableDataCell>
                                      <CButton
                                        color="warning"
                                        component="a"
                                        href={`/staff/selected/filterbyCourse/unicode/students?course=${courseID}&university=${item[0]}`}
                                      >
                                        View More
                                      </CButton>
                                    </CTableDataCell>
...
export default SelectedByUnicode
